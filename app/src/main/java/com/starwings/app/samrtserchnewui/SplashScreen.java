...
public class SplashScreen extends AppCompatActivity {
        ...
        if(appPreferences.getString("apikey","NA")=="NA")
        {
            ...
            registerClient.post(ApiLinks.baseLink+ApiLinks.register,registerParams, new AsyncHttpResponseHandler() {
                ...
                public void onFailure(int statusCode, Header[] headers, byte[] responseBody, Throwable error) {
                        showFailure(error.getClass().getName().toString().getBytes());
                }
            });
        }
        else
        {
            Intent i = new Intent(SplashScreen.this, HomeWrapperPage.class);
            startActivity(i);
            loadProgress.setVisibility(View.GONE);
            finish();
        }
    ...
    private void showFailure(byte[] error) {
        ...
        loadProgress.setVisibility(View.GONE);
        Toast.makeText(this,"Failure. Try Later"+new String(error),Toast.LENGTH_SHORT).show();
    }
